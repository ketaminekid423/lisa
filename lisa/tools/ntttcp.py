...
class Ntttcp(Tool):
    ...
    output_pattern = re.compile(
        ...
        r"throughput.*:(?P<throughput_in_gbps>.+)Gbps(([\w\W]*?)"
    ...
    def run_as_server_async(
        ...
        process = self.node.execute_async(
            f"ulimit -n 204800 && {self.command} {cmd}", shell=True
        )
        return process
    ...
    def create_ntttcp_result(
        ...
        ntttcp_result.throughput_in_gbps = Decimal(
            matched_results.group("throughput_in_gbps")
        )
